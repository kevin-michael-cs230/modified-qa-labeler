...
class QuestDataset(Dataset):
    ...
    def _pad_and_truncate(self, features, pad_value=0):
        ...
        features = features + [pad_value, ] * \
            (self.max_seg_length - len(features))
...
def get_model(bert_config_json_path, targets=ALL_TARGETS):
...
def predict_test_checkpoints(checkpoints, test_loader, targets, bert_config_json_path, device="cuda"):
    model = get_model(bert_config_json_path, targets)
    pred = []
...
def predict_frame(df, tokenizer_path, checkpoints, bert_config_json_path, batch_size, num_workers):
    ...
    return predict_test_checkpoints(checkpoints, dataset_loader, ALL_TARGETS, bert_config_json_path)
